...
- **Attach:** a subtle `Paperclip` icon button leads the toolbar. Its tooltip gives the model's media gate and the limits (images 3 MiB, PDF 10 MiB, text 256 KiB, 5 per message). When the model or the Task state refuses uploads, it stays visible, dimmed, `aria-disabled`, and says why. Paste and drag-and-drop also work; while files are dragged over the composer, a `raised` overlay with an inset dashed `accent` outline shows "Drop to attach" and the same note (`fade-in`).
