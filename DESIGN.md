...
- **Pickers** at left: Model (`Cpu`, value in `code-sm`), combined Effort and Context (`Gauge`, e.g. "high · 200K"), and Mode (`Shield`; `ShieldOff` in `attention` for yolo), separated by hairlines. The combined menu has two named sections; unsupported choices show their reason. Model/effort/context changes stay disabled during a turn. Mode can change during a turn. Read-only values remain visible and cannot change.
