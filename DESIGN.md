...
- **Turn status:** one 34px row, with no rule under it (nothing full-width separates turns; the user bubble and the whitespace do), heads every assistant turn, under the user bubble. Live, it carries the working mark and "Working for 12s" from the recorded foreground start (steering does not reset it; without a trustworthy start, "Working"); ended, the same row reads "Worked for 12s" from the recorded duration, or stays blank when none was recorded. Streamed content lands below it, so nothing on screen moves when a turn starts or ends. Background shells remain independently visible above the composer and never drive foreground timing.
...
- **Transcript:** new rows rise in, tool rows one by one as they arrive; streaming text appends without animation; the turn status row keeps its slot from "Working" to "Worked"; "Thinking…" shimmers `muted` → `body`; the working mark orbits and breathes; the locate action flashes `accent-wash` for 1.4s; a rendered diagram fades in (`base`) over the code it replaces; the "New output" button appears and leaves through `Appear`.
