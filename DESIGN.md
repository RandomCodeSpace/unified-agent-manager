...
4. **Surfaces step and float, they do not stack.** Depth = one shade lighter and one level up the elevation ladder; the ladder's 1px ring is the only edge a surface has. Shadows are cool ink (`rgba(20,28,45,α)`) on this cool canvas, two layers at most, and are never animated: a hover lift or a focus glow fades a pre-drawn shadow in through opacity.
...
### Buttons and inputs
Primary is ink; secondary is a filled `sunken` surface with no edge (`tint-hover` on hover, `hairline` pressed); ghost has no fill; danger is `error` text with `error-wash` hover; icon buttons are 28px (32 in headers) `muted` glyphs. Disabled is 45% opacity. Inputs and select triggers are 36px filled `sunken` surfaces with the inset `well` ring and no edge; focus is the `focus` glow (a 1px `focus` edge at 6.7:1 on `raised` with a 4px halo at 18%) in place of the outline; `aria-invalid` turns the ring `error`. Labels sit above in `caption` `muted`. On the boundary rule of WCAG 1.4.11: a field at rest is identified by its label, its placeholder or value and its caret rather than by its fill (1.12:1 against `raised`), and its focused edge is well above 3:1; a secondary button is identified by its label. This is a deliberate trade for the borderless surface and is recorded here.
...
- **Focus:** one glow-style ring. `:focus-visible` is the 2px `focus` outline, 1px out, with a 5px halo at 18% behind it (`box-shadow`); a surface with its own shadow utility keeps its shadow and shows the outline alone, and inside rows the ring is inset. Fields and the composer replace the outline with the `focus` glow (a 1px `focus` edge and the same halo), which is the same ring with a thinner edge. Base UI traps focus in dialogs and the drawer and returns it to the opener.
...
- Don't animate a shadow, a filter or a size, add `backdrop-filter`, mask a scroll container, or add per-row shadows to a long list; lift and glow through a pseudo-element's opacity, and fade scroll edges with small overlays.
