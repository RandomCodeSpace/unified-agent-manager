...
func TestFinalAcceptanceProjectPersistenceFailureKeepsPublishedState(t *testing.T) {
	...
	if _, err := m.UpdateProject(id, "unsaved rename"); err == nil {
	...
	if _, err := m.UpdateProject(id, "must not resurrect"); statusOf(err) != http.StatusNotFound {
...
func TestFinalAcceptanceCreateInputFailureNeverOpensProvider(t *testing.T) {
	...
	if _, err := m.UpdateProject(project, invalidName); statusOf(err) != http.StatusBadRequest {
