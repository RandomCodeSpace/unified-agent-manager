...
func TestViewerDisconnectMidTurnLeavesProviderRunning(t *testing.T) {
	...
	if _, err := m.Submit(sum.ID, "do it", mustUUID(t)); err != nil {
		t.Fatal(err)
	}
...
func TestDuplicateRequestIDSendsOnce(t *testing.T) {
	...
	if _, err := m.Submit(sum.ID, "hello", "not-a-uuid"); statusOf(err) != http.StatusBadRequest {
...
func TestConcurrentSubmitsOneAcceptedOneConflict(t *testing.T) {
	...
	for _, text := range []string{"first", "second"} {
		...
		go func() {
			defer wg.Done()
			sub, err := m.Submit(sum.ID, text, mustUUID(t))
...
func TestSubmissionOutcomes(t *testing.T) {
	...
	rid := mustUUID(t)
	sub, err := m.Submit(sum.ID, "maybe", rid)
	...
	if again, _ := m.Submit(sum.ID, "maybe", rid); again != sub || len(conv.Sends()) != 1 {
	...
	if _, err := m.Submit(sum.ID, "busy", mustUUID(t)); statusOf(err) != http.StatusConflict {
	...
	sub, err = m.Submit(sum.ID, "rejected", mustUUID(t))
	...
	conv.EmitTurn(agentapi.TurnWorking, "")
	if _, err := m.Submit(sum.ID, "while working", mustUUID(t)); statusOf(err) != http.StatusConflict {
...
func TestCancelCallsCancelOnly(t *testing.T) {
	...
	if _, err := m.Submit(sum.ID, "work", mustUUID(t)); err != nil {
		t.Fatal(err)
	}
	conv.EmitTurn(agentapi.TurnWorking, "")
...
func TestShutdownInterruptsClosesAndShutsDownProviders(t *testing.T) {
	...
	again, err := m2.Submit(sum.ID, "work", rid)
