...
const tmuxSetOption = "set-option"
...
func (c *Client) SetSessionLabel(ctx context.Context, session, label, window string) error {
	if _, err := c.run(ctx, tmuxSetOption, "-t", session, "@uam_label", label); err != nil {
...
// server: enable mouse mode so wheel events scroll tmux pane history instead
// of leaking to full-screen prompts as history navigation, keep OSC 52
// clipboard sync enabled for tmux-side copies, and swallow Ctrl+Z so
// it can't suspend the agent in the foreground pane.
//
...
func (c *Client) applyServerConfig(ctx context.Context) error {
	...
	for _, opt := range [][2]string{
		...
		if out, err := c.run(ctx, tmuxSetOption, "-g", opt[0], opt[1]); err != nil {
