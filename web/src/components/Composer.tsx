...
export function Composer({ session, onSessionUpdate }: { session: SessionDetail; onSessionUpdate: (s: SessionSummary) => void }) {
  ...
  return (
    <form
      className={cn(
        'flex flex-col rounded-md border border-hairline bg-raised shadow-[0_1px_2px_rgba(28,27,24,0.05)] transition-[border-color,box-shadow] duration-160 focus-within:border-hairline-strong focus-within:shadow-[0_2px_8px_rgba(28,27,24,0.08)]',
        locked && 'bg-surface',
      )}
      onSubmit={(e) => {
        e.preventDefault();
